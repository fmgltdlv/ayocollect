...
async function digAlertTicketId(
  ...
  const row = await db
    ...
    .prepare('SELECT id FROM dig_alert_tickets WHERE ticket_number = ? AND revision = ?')
...
async function usanTicketId(db: D1Database, table: UsanTable, ticketNumber: string): Promise<number | null> {

  const row = await db

    .prepare(`SELECT id FROM ${table}_tickets WHERE ticket_number = ?`)
...
export async function upsertDigAlert(db: D1Database, envelope: DigAlertPayload): Promise<string | null> {
  ...
  const hadLate = scanHadLateResponse(responseCodes) ? 1 : 0;
  ...
  await db.prepare('DELETE FROM dig_alert_revisions WHERE ticket_number = ?').bind(ticketNumber).run();
  ...
  await db
    ...
    .prepare(
      ...
      `INSERT INTO dig_alert_tickets (
        ...
        ticket_number, revision, api_timestamp,
        ...
        work_type, work_order, one_year, centroid_x, centroid_y,

        polygon_wkt, bbox_min_lon, bbox_min_lat, bbox_max_lon, bbox_max_lat,

        had_late_response, updated_at
  ...
  const ticketId = await digAlertTicketId(db, ticketNumber, revision);

  if (!ticketId) return null;



  await db.prepare('DELETE FROM dig_alert_responses WHERE ticket_id = ?').bind(ticketId).run();
    ...
    await db
      ...
      .prepare(
        ...
        `INSERT INTO dig_alert_responses (
          ...
          ticket_id, utility_code, utility_name, response_code,
...
export async function upsertUsan(
  ...
  const ticketId = await usanTicketId(db, table, ticketNumber);
  ...
  await db.batch([
    ...
    db.prepare(`DELETE FROM ${table}_stations WHERE ticket_id = ?`).bind(ticketId),

    db.prepare(`DELETE FROM ${table}_ticket_history WHERE ticket_id = ?`).bind(ticketId),
    ...
    await db
      ...
      .prepare(
        ...
        `INSERT INTO ${table}_stations (
          ...
          ticket_id, name, code, response_date, response_date_string,
    ...
    await db
      ...
      .prepare(
        ...
        `INSERT INTO ${table}_ticket_history (
          ...
          ticket_id, revision_suffix, name, code, response_date, response_date_string,
      ...
      .bind(
        ...
        usanRevisionSuffix(str(h.requestNumber)),
