...
# System configuration - will be set based on --system argument
# Initialize to CA by default
POSR_API = None
MAP_URL = None

def get_system_urls(system: str = "CA") -> Tuple[str, str]:
    """
    Returns (POSR_API, MAP_URL) tuple for the specified system.
    system: "CA" or "NV"
    """
    system = system.upper()
    if system == "NV":
    ...
    return posr_api, map_url

def set_system(system: str = "CA"):
    """Sets the global POSR_API and MAP_URL based on system choice."""
    global POSR_API, MAP_URL
    POSR_API, MAP_URL = get_system_urls(system)
...
def _launch_gui():
    ...
    # Initialize system to CA by default
    set_system("CA")

    # --- Vars ---
    system_var = tk.StringVar(value="CA")
    ...
    def run_sequential():
        ...
        # Set system before running
        selected_system = system_var.get()
        set_system(selected_system)
        log(f"🌐 Using system: {selected_system}")
        
        # Disable controls
        is_running = True
        stop_flag["value"] = False
        run_btn.configure(state="disabled")
        system_combo.configure(state="disabled")
        ...
        def run_in_thread():
            try:
                ...
                root.after(0, lambda: system_combo.configure(state="normal"))
    ...
    ttk.Label(frm, text="System:").grid(row=1, column=0, sticky="w", pady=(0, 5))
    system_combo = ttk.Combobox(frm, textvariable=system_var, values=["CA", "NV"], state="readonly", width=12)
    system_combo.grid(row=1, column=1, sticky="w", padx=6, pady=(0, 5))
    ttk.Label(frm, text="CA (California) or NV (Nevada)", font=("", 8)).grid(row=1, column=2, sticky="w", pady=(0, 5))

    # Row 2: Start date
    ttk.Label(frm, text="Start Date (YYYY-MM-DD):").grid(row=2, column=0, sticky="w", pady=(5, 5))
    date_entry = ttk.Entry(frm, textvariable=start_date_var, width=15)
    date_entry.grid(row=2, column=1, sticky="w", padx=6, pady=(5, 5))
    ttk.Label(frm, text="Format: YYYYMMDDXXXXX (e.g., 2025010100001)", font=("", 8)).grid(row=2, column=2, sticky="w", pady=(5, 5))

    # Row 3: End date
    ttk.Label(frm, text="End Date (YYYY-MM-DD, optional):").grid(row=3, column=0, sticky="w", pady=(5, 5))
    end_date_entry = ttk.Entry(frm, textvariable=end_date_var, width=15)
    end_date_entry.grid(row=3, column=1, sticky="w", padx=6, pady=(5, 5))
    ttk.Label(frm, text="Leave empty to continue indefinitely", font=("", 8)).grid(row=3, column=2, sticky="w", pady=(5, 5))
    ...
    # Row 8: Output label
    ttk.Label(frm, text="Output:", font=("", 9, "bold")).grid(row=8, column=0, columnspan=3, sticky="w", pady=(5, 2))

    # Row 9: Output box
    output_frame = ttk.Frame(frm)
    output_frame.grid(row=9, column=0, columnspan=3, sticky="nsew")
...
if __name__ == "__main__":
    import argparse
    import sys
    parser = argparse.ArgumentParser(description="Sequential ticket scraper for underground service alerts (CA or NV).")
    ...
    parser.add_argument("--system", choices=["CA", "NV"], default="CA", help="System to use: CA (California) or NV (Nevada). Default: CA")
    args = parser.parse_args()
    
    # Initialize system URLs based on --system argument
