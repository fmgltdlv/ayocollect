...
export async function upsertPosrPayload(
  ...
  await db
    .prepare(
      `INSERT INTO posr_fetches (ticket_base, trail_id, is_successful, validation_errors, raw_r2_key)
       VALUES (?, ?, ?, ?, ?)`,
    )
    .bind(
      ticketBase,
...
export async function upsertDigalertPayload(
  ...
  await db
    ...
    .bind(ticketBase, payloadR2Key ?? null)
    .run();
  ...
  await db
    .prepare(
      `INSERT INTO ticket_polygons (
        ...
        bbox_min_lat, bbox_max_lat, bbox_min_lon, bbox_max_lon, gml_r2_key, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
      ON CONFLICT(request_number) DO UPDATE SET
        ...
        gml_r2_key = excluded.gml_r2_key,
        updated_at = datetime('now')`,
    )
    .bind(
      requestNumber,
